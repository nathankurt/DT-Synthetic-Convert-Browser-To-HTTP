...
def get_eligible_locations(http_locations, browser_locations,b_id):
    ...
    for location in monitor_obj.b_json["locations"]:
        ...
        if location in [http["entityId"] for http in http_locations]:
            ...
            logger.warn(f"Browser Monitor {b_id} using location {location} which either isn't supported yet for HTTP Monitors or Does not exist")
...
#Finish and run the script again without doing anything
    ...
    if args.select_monitor_id:
        ...
        logger.critical("Input Error: Must Select Filter for monitors, if you want all monitors, make sure to use -a")
    ...
    assert browser_monitor_ids != [], logger.critical("Assertion Error, No Monitors in this scope")
    ...
    for b_id in browser_monitor_ids:
        ...
        if b_monitor_type != "availability":
            logger.warn(f"Browser Clickpath ID: {b_id} Not supported, skipping")
            continue
        ...
        if not args.location:
            ...
            if not loc_list:
                logger.critical(f"Browser Monitor {monitor_obj.b_json['name']}(ID: {b_id}) Doesn't have any locations set that are available for public http monitors, you can set a location in the arguments")
                continue
        else:
            loc_list = get_eligible_locations(http_locations, args.location, b_id)
            if not loc_list:
                logger.critical(f"Monitor Locations You Chose are either ineligible or don't exist, check ids that do exist by using the --list command")
                continue
        ...
        for event in monitor_obj.b_json["script"]["events"]:
            ...
            if "authentication" in event:
                if event["authentication"]["type"] == "webform":
                    logger.warn("Authentication Type is Webform, Skipping Monitor")
                    break
        ...
        if not args.dry_run:
            ...
            if response.status_code >= 400:
                ...
                logger.error(f"can't post monitor Name: {monitor_obj.b_json['name']}, ID: {b_id}")
            ...
            if args.overwrite and response.status_code < 400 and del_id != "":
                ...
                assert del_response.status_code < 400, logger.error(f"Unable to delete monitor {del_id}\
                    Error Code: {del_response.status_code}")
                logger.info(f"Overwrote HTTP Monitor: {del_id}")


            
            if args.delete and response.status_code < 400:
                del_b_monitor_response = monitor_obj.delete_monitor()
                assert del_b_monitor_response.status_code < 400, logger.error(f"Unable to delete browser monitor {b_id}\
                    Error Code: {del_b_monitor_response.status_code}")
                logger.info(f"Deleted Old Browser Monitor, Name: {monitor_obj.b_json['name']} ID: {b_id}")
            elif args.disable and response.status_code < 400:
                disable_b_monitor_response = monitor_obj.disable_monitor()
                assert disable_b_monitor_response.status_code < 400, logger.error(f"Unable to disable browser monitor {b_id} \
            ...
            logger.debug("HTTP ID: " + http_id)
            b_monitor_http_monitor_dict.update({b_id:http_id})

            logger.info(f"Creating Monitor from Old Browser Monitor ID: {b_id} New Browser Monitor ID: {http_id}")

        #empty list so no requirements met. 

        assert b_monitor_http_monitor_dict.keys(), logger.critical("Error: No Monitors in this scope")
    ...
    if len(failed_monitors.keys()) > 0:
        ...
        logger.info("All Monitors Successfully Created")
